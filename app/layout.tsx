      ...
      <body className={`${inter.variable} font-body-md bg-background text-on-surface min-h-screen pb-[80px]`}>
        ...
        <nav className="fixed bottom-0 w-full h-[56px] z-50 bg-surface shadow-[0px_-2px_8px_rgba(0,0,0,0.05)] md:hidden">
          <div className="flex justify-around items-center w-full max-w-[768px] mx-auto px-margin-mobile h-full">
            <button className="flex flex-col items-center justify-center text-secondary relative after:content-[''] after:absolute after:-bottom-1 after:w-1 after:h-1 after:bg-secondary after:rounded-full hover:opacity-80 transition-opacity active:scale-90 transition-transform">
            ...
            <button className="flex flex-col items-center justify-center text-on-surface-variant hover:text-secondary transition-colors active:scale-90 transition-transform">
              <span className="material-symbols-outlined">checklist</span>
              <span className="font-label-sm text-label-sm mt-1">Tareas</span>
            </button>
            <button className="flex flex-col items-center justify-center text-on-surface-variant hover:text-secondary transition-colors active:scale-90 transition-transform">
              <span className="material-symbols-outlined">calendar_month</span>
              <span className="font-label-sm text-label-sm mt-1">Calendario</span>
            </button>
            <button className="flex flex-col items-center justify-center text-on-surface-variant hover:text-secondary transition-colors active:scale-90 transition-transform">
